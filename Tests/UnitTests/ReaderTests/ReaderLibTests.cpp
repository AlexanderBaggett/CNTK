...
BOOST_AUTO_TEST_CASE(TestCompareChunkAndSampleBasedRandomizations)
{
    auto num_chunks = 10;
    ...
    for (int k = 3; k <= 3; k++) 
    {
        ...
        for (int i = 0; i < randomizedChunks1.size(); i++)
        {
            ...
            //BOOST_CHECK(a.m_randomizationWindow == b.m_randomizationWindow);
        }
    }
}
...
//    auto mockDeserializer = make_shared<MockDeserializer>(num_chunks, num_sequences, input);
//
//    BlockRandomizer randomizer(0, 3, mockDeserializer, /*prefetch =*/ false, 
